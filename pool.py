...
def message_on_pool_state_change(pool_settings, optiondict, pool_heater_allowed):
    ...
    if not pool_settings:
        ...
        if os.path.isfile(optiondict["pool_missing_filename"]):
            ...
            pool_days, pool_seconds = modification_days_and_seconds(
                optiondict["pool_missing_filename"]
            )
        ...
        # log message
        logger.info(
            "Not reading pool settings - sent message: %s and created file: %s",
            msgid["id"],
            optiondict["pool_missing_filename"],
        )
        ...
        return
    elif optiondict["pool_missing_filename"] and os.path.exists(
        optiondict["pool_missing_filename"]
    ):
        ...
        # log message
        logger.info(
            "NOW reading pool settings - sent message: %s and removed file: %s",
            msgid["id"],
            optiondict["pool_missing_filename"],
        )
    ...
    if os.path.isfile(optiondict["pool_heater_filename"]):
        ...
        pool_days, pool_seconds = modification_days_and_seconds(
            optiondict["pool_heater_filename"]
        )
        ...
        if pool_settings["pool_heat_mode"] == "Off":
            ...
            # log message
            logger.info(
                "Pool heater off - sent message: %s and removed file: %s",
                msgid["id"],
                optiondict["pool_heater_filename"],
            )
            ...
            # log message
            logger.info(
                "Pool heater still ON [%s] days - sent message: %s and removed file: %s",
                pool_days,
                msgid["id"],
                optiondict["pool_heater_filename"],
            )
        elif (
            pool_settings["pool_temp_set"]
            and float(pool_settings["pool_temp_set"]) > MAX_POOL_TEMP
        ):
            ...
            msgid = kvgmailsendsimple.gmail_send_simple_message(
                optiondict["pool_email_from"],
                optiondict["pool_email_to"],
                optiondict["pool_email_subject"]
                + "SET OVER THE MAX SETTING:  "
                + str(MAX_POOL_TEMP),
                "Pool Heater set to a temp "
                + pool_settings["pool_temp_set"]
                + " that is over MAX SETTING:  "
                + str(MAX_POOL_TEMP)
                + pool_ok,
                optiondict["scopes"],
                optiondict["file_token_json"],
            ...
            # log message
            logger.info(
                "Pool heater set over max [%s/%s] days - sent message: %s and removed file: %s",
                pool_settings["pool_temp_set"],
                str(MAX_POOL_TEMP),
                msgid["id"],
                optiondict["pool_heater_filename"],
        ...
        if pool_settings["pool_heat_mode"] != "Off":
            ...
            # log message
            logger.info(
                "Pool heater ON - sent message: %s and created file: %s",
                msgid["id"],
                optiondict["pool_heater_filename"],
            )
            ...
            if (
                ...
                if optiondict["pool_ip"]:
                    asyncio.run(
                        poolapi.set_pool_temp(
                            ip=optiondict["pool_ip"], desired_temp=MAX_POOL_TEMP
                        )
                    )
...
def message_on_pool_turn_off(
    pool_settings, pool_heater_allowed, pool_heater_invalid_dates, optiondict
):
    ...
    if datetime.datetime.now().date() in pool_heater_allowed:
        # log message
        logger.info(
            "Pool heater ON and pool_heater_allowed is enabled - no action taken"
        )
        return
    ...
    if pool_settings["pool_heat_mode"] != "Off":
        ...
        if optiondict["direct_connect"] and optiondict["pool_ip"]:
            ...
            asyncio.run(
                poolapi.set_heat_mode_pool(
                    ip=optiondict["pool_ip"], mode=0, logger=logger
                )
            )
        else:
        ...
        # log message
        logger.info(
            "Pool heater ON - being turned OFF sent message: %s and created file: %s",
            msgid["id"],
            optiondict["pool_heater_off_filename"],
        )
...
def message_on_spa_state_change(pool_settings, optiondict):
    ...
    if not pool_settings:
        ...
        if os.path.isfile(optiondict["spa_missing_filename"]):
            # check the age/duraction of this file to see if we should send again
            spa_days, spa_seconds = modification_days_and_seconds(
                optiondict["spa_missing_filename"]
            )
        ...
        # return - we have nothing to process
        return
    elif optiondict["spa_missing_filename"] and os.path.exists(
        optiondict["spa_missing_filename"]
    ):
        # we are getting data and we have a lock file that must now be removed
    ...
    if os.path.isfile(optiondict["spa_heater_filename"]):
        # if there is a lock file - capture the informatoin about this lock file
        spa_days, spa_seconds = modification_days_and_seconds(
            optiondict["spa_heater_filename"]
        )
        ...
        if pool_settings["spa_heat_mode"] == "Off":
            ...
            # log message
            logger.info(
                "SPA heater off - sent message: %s and removed file: %s",
                msgid["id"],
                optiondict["spa_heater_filename"],
            )

        elif (
            optiondict["spa_heater_off_filename"]
            and optiondict["spa_heater_off_hours"]
            and spa_seconds > optiondict["spa_heater_off_hours"] * 60 * 60
        ):
            ...
            # log message
            logger.info(
                "SPA heater on too long turning off SPA - sent message: %s and created file: %s",
                msgid["id"],
                optiondict["spa_heater_off_filename"],
            )
            ...
            # log message
            logger.info(
                "SPA heater still ON [%s] days - sent message: %s and removed file: %s",
                spa_days,
                msgid["id"],
                optiondict["spa_heater_filename"],
            )
        ...
        if pool_settings["spa_heat_mode"] != "Off":
            ...
            # log message
            logger.info(
                "SPA heater ON - sent message: %s and created file: %s",
                msgid["id"],
                optiondict["spa_heater_filename"],
            )
...
if __name__ == "__main__":
    ...
    if optiondict["direct_connect"]:
        ...
        pool_settings = read_direct_output_pool(
            optiondict["pool_ip"], optiondict["pool_filename"]
        )
    else:
        pool_settings = read_parse_output_pool(
            optiondict["input_filename"], optiondict["pool_filename"]
        )

    # POOL - capture valid dates for pool to be enabled
    pool_heater_allowed, pool_heater_invalid_dates = (
